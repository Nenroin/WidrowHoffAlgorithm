...
double learning_data::get_training_val_at_any_idx(const unsigned long long int idx) const
{
    const unsigned long long int wrapped_idx{idx % training_data_.size()};
    return training_data_.at(wrapped_idx);
}

double learning_data::get_test_val_at_any_idx(const unsigned long long int idx) const
{
